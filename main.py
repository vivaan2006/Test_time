...
class Main(object):
  ...
  """
    ...
    # This sets up the main frame of the system once the login verification has been passed
    ...
    def show_notifications(self):
        ...
        cursor.execute("SELECT * FROM NOTIFICATIONS WHERE first_name = ?", (self.first_name,))
    ...
    def admin_output_reports(self):
        ...
        self.filter_graph_combobox.addItem("Gender Distribution")
        self.filter_graph_combobox.addItem("Points by Gender")
        ...
        self.download_button.setGeometry(30, 350, 150, 50)
        ...
        self.download_leaderboard_button.setGeometry(30, 420, 150, 50)
    ...
    def generate_graph(self, index):
        ...
        if selected_option == "Points by Grade":
            ...
            cursor.execute("SELECT GENDER, COUNT(*) FROM main.students GROUP BY GENDER;")
            result = cursor.fetchall()
            ...
            colors = plt.cm.get_cmap('tab10')
            color_values = colors(np.linspace(0, 1, len(genders)))
            ax.pie(counts, labels=genders, colors=color_values, autopct='%1.1f%%')

        elif selected_option == "Points by Gender":
            sqliteConnection = sqlite3.connect('identifier.sqlite')
            cursor = sqliteConnection.cursor()

            cursor.execute("SELECT GENDER, SUM(points) FROM main.students GROUP BY GENDER;")
            result = cursor.fetchall()
            ...
            plt.xlabel("Gender")
            plt.ylabel("Points")
            plt.title("Points by Gender")

            colors = plt.cm.get_cmap('tab10')
            color_values = colors(np.linspace(0, 1, len(genders)))
            ax.bar(genders, points, color=color_values)
    ...
    def resolve_question(self, text_edit):
        ...
        button = text_edit.parentWidget().findChild(QPushButton, "resolved_button")
        button.deleteLater()
...
class Slideshow(QRunnable):
    ...
    def run(self) -> None:
        ...
        title_list = [
            "Canyons District was voted the most sanitary and safe school district in the state!\n",

            "Hillcrest High School has the highest graduation rate and most challenging curriculum!\n",

            "Movie Night at the Canyons School District Main Office!\n",
        ...
        description_list = [
            "With routine cleaning and sanitizing, Hillcrest High School was voted the most sanitary and safe school",

            "Hillcrest High School was awarded the most challenging curriculum in the district! Leading to a more "
            "prepared student body! Not only was the curriculum challenging, but the school had the highest "
            "graduation rate in the district",

            "District Film Festival will be held on the second week of the 2023-2024 school year, where the school will be hosting the new Transformers movie!",
            ...
            "Hillcrest High library renovations lead to 1000 students issuing books from the library almost every month, which is almost half the school population, and a feat that very few High schools could accomplish!",
        ]
